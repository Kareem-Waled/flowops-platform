...
echo "===== Jenkins FlowOps Production Deploy ====="
...
BUILD_TAG_VALUE="prod-${BUILD_NUMBER:-manual-$(date +%Y%m%d%H%M%S)}"
...
echo "Updating Kubernetes production manifests..."

sed -i "s|image: .*user-service:.*|image: ${USER_IMAGE}|g" \
  k8s/manifests/production/user-service/deployment.yaml

sed -i "s|image: .*product-service:.*|image: ${PRODUCT_IMAGE}|g" \
  k8s/manifests/production/product-service/deployment.yaml

echo "Updated image lines:"
grep -RIn "image:" k8s/manifests/production/user-service/deployment.yaml k8s/manifests/production/product-service/deployment.yaml

echo "Git diff:"
git diff -- k8s/manifests/production

git config user.email "jenkins@flowops.local"
git config user.name "Jenkins CI"

git add k8s/manifests/production/user-service/deployment.yaml
git add k8s/manifests/production/product-service/deployment.yaml

if git diff --cached --quiet; then
  echo "No manifest changes to commit."
else
  git commit -m "Deploy production images ${BUILD_TAG_VALUE}"
...
echo "===== Jenkins FlowOps Production Deploy Completed ====="
